...
public class QuickSort {
    ...
    public int findKthElementLogkTime(int[] arr1, int[] arr2, int k) {
        int i = 0, j = 0, rank = 0;
        while (rank < k) {
            int lastElement = 0;
            if (arr1[i] < arr2[j]) {
    ...
    /**
     * Find median with sorted arrays.
     * @param arr1
     * @param arr2
     * @return
     */
    public int findMedian(int[] arr1, int[] arr2) {
        ...
        int n1 = arr1.length, n2 = arr2.length, k = (n1+n2+1)/2;
        int low = 0, high = n1;
        while (low<=high) {
    ...
    public int findMajorityElement(int[] arr) {
        ...
        if (majorityCount > arr.length / 2)
            return prev;
        throw new NoSuchElementException("No majority element exists");
    }
}
